...
class Acedrg(CExeCode ):
    ...
    def checkDependency(self):
        ...
        print("acedrgDir is at ", self.acedrgDir)
        ...
        if self.workMode == 12 or self.workMode == 121 or self.workMode==52 :
            ...
            if os.path.isfile(self.inSmiName) : 
                if self.chemCheck.checkCarbonMolFromSmi(self.inSmiName):
                    ...
                    self.processComboMolContainCarBoFromSmi(initAtoms, initBonds)
        ...
        if self.workMode in [11,  111, 112, 114, 51] :  #  and not self.isAA :
            ...
            for iMol in range(len(self.rdKit.molecules)):
                ...
                if not os.path.isfile(self.nameFile):
                    print("here2")
                    self.rdKit.MolToSimplifiedMmcif(self.rdKit.molecules[iMol], self.inMmCifName, self.chemCheck, 
                                                    self.monomRoot, self.fileConv.chiralPre, self.fileConv.chiralBoth)
                else:
                    print("here3")
                    self.rdKit.MolToSimplifiedMmcif(self.rdKit.molecules[iMol], self.inMmCifName, self.chemCheck, 
                                                    self.monomRoot, self.fileConv.chiralPre, self.fileConv.chiralBoth, 
    ...
    def processComboMolContainCarBoFromSmi(self, tInitAtoms, tInitBonds):
                ...
                if len(self.chemCheck.aSetNewMols) > 0:
                    aComboMol = {}
                    self.chemCheck.assembleNewMods2(tInitAtoms, tInitBonds, 
                                                   aSetNonCBMols, aSetCBMols, self.scrDir, self.monomRoot, aComboMol)
